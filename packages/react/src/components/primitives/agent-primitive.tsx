...
import { useAgent, useAgentCapabilities, useConnection } from '../../hooks/use-agent'
      ...
      <div ref={ref} {...props}>
        {children || (
          <div>
            ...
            <div data-feature="push" data-supported={supportsPushNotifications()}>
...
const AgentIf = forwardRef<HTMLDivElement, AgentIfProps>(
  ({ 
    ...
    const { isConnected, supportsStreaming, supportsPushNotifications } = useAgent()
    const capabilities = useAgentCapabilities()
    ...
    if (hasCapabilities !== undefined && (capabilities.length > 0) !== hasCapabilities) return null

    // Check streaming support condition
    if (supportsStreamingProp !== undefined && supportsStreaming() !== supportsStreamingProp) return null

    // Check push notifications support condition
    if (supportsPushProp !== undefined && supportsPushNotifications() !== supportsPushProp) return null
